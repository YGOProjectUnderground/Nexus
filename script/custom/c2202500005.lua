...
function s.regcon(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500001),tp,LOCATION_FZONE,LOCATION_FZONE,1,nil)
end

function s.quickcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500001),tp,LOCATION_FZONE,LOCATION_FZONE,1,nil)
end

function s.fzfilter(c,tp)
	return c:IsCode(2202500001) and c:GetActivateEffect() and c:GetActivateEffect():IsActivatable(tp,true,true)
end
...
function s.gytg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500001),tp,LOCATION_FZONE,LOCATION_FZONE,1,nil) then
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
	end
end
...
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(g,REASON_EFFECT)==2 
		...
		and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500001),tp,LOCATION_FZONE,LOCATION_FZONE,1,nil)
