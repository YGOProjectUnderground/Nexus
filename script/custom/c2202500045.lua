...
function s.initial_effect(c)
	...
	e2:SetCost(s.cost)
	c:RegisterEffect(e2)
	local e3=Ritual.CreateProc({handler=c,desc=aux.Stringid(id,2),lvtype=RITPROC_EQUAL,filter=aux.FilterBoolFunction(Card.IsCode,2202500148),location=LOCATION_GRAVE})
	c:RegisterEffect(e3)	
	local e4=Ritual.CreateProc({handler=c,desc=aux.Stringid(id,3),lvtype=RITPROC_EQUAL,filter=aux.FilterBoolFunction(Card.IsCode,2202500148),extrafil=s.extragroup,
...
function s.condition1(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500154),tp,LOCATION_FZONE,0,1,nil)
end
function s.condition2(e,tp,eg,ep,ev,re,r,rp)
	...
	return e:GetHandler():GetCode()==id and #fg>0 and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,2202500154),tp,LOCATION_FZONE,0,1,nil)
end
