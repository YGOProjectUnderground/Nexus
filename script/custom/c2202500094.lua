...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			...
			if tc and Duel.IsPlayerCanSpecialSummonMonster(tp,2202500096,0,0,0,0,tc:GetLevel(),tc:GetRace(),tc:GetAttribute()) then 
				Duel.ConfirmCards(1-tp,tc) 
				local token=Duel.CreateToken(tp,2202500096)
