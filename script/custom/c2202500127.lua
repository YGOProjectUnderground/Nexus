...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)	
	local e2=Ritual.CreateProc({handler=c,desc=aux.Stringid(id,1),lvtype=RITPROC_GREATER,filter=aux.FilterBoolFunction(Card.IsCode,2202500126),location=LOCATION_DECK})
	c:RegisterEffect(e2)		
...
s.fit_monster={2202500126}
s.listed_names={2202500126}
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,59822133)
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,2202500130,0x10aa,0x4011,1800,1000,4,RACE_MACHINE,ATTRIBUTE_EARTH) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,2202500130,0x10aa,0x4011,1800,1000,4,RACE_MACHINE,ATTRIBUTE_EARTH) then return end
	for i=1,2 do
		local token=Duel.CreateToken(tp,2202500130)
