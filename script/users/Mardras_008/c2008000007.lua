--Greater Imprisoned Queen Archfiend
Duel.LoadScript("_load_.lua")
local s,id=GetID()
function s.initial_effect(c)
    --link sum
    c:EnableReviveLimit()
	--Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsCode,52248570),1,1)
...
function s.lcheck(g,lc,sumtype,tp)
	return g:IsExists(Card.IsSummonCode,1,nil,lc,sumtype,tp,52248570)
end
...
function s.rmfilter(c)--rm 1 c your opp controls, face-down
	return c:IsSetCard(0x45) and c:IsAbleToRemoveAsCost() and c:IsAbleToRemoveAsCost()
end
...
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)--atk up
	return Duel.IsEnvironment(2008000020) and Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function s.filter(c)
	return c:IsType(TYPE_LINK) and c:GetLink()==1 and c:IsRace(RACE_FIEND)
end
