...
function s.cfilter(c)--act/add Sanctuary from D/GY
...
function s.actfilter(c,e,tp,code)
	return c:IsCode(2008000064) and (c:IsAbleToHand() or c:GetActivateEffect():IsActivatable(tp,true,true))
end
...
function s.actop(e,tp,eg,ep,ev,re,r,rp)
       ...
       function(c)
       aux.PlayFieldSpell(tc,tp,tp,eg,ep,ev,re,r,rp)
    end,
   aux.Stringid(id,2))
end