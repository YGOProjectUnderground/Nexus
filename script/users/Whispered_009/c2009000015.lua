...
--banish then Special Summon with different type
function s.cfilter(c,g,rc)
  local rc=c:GetRace()
	return rc~=0 and g:IsContains(c) and c:IsRace(RACE_AQUA|RACE_SEASERPENT|RACE_FISH) and c:IsAbleToRemove() and c:IsRace(rc)
end
