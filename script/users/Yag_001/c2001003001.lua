--Tisiphone the Warden of Torment
...
function s.rsop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if #mat>0 then
        ...
        if Duel.SpecialSummon(c,SUMMON_TYPE_RITUAL,tp,tp,false,true,POS_FACEUP)>0 then
            --Add equip effect here
            local g=Duel.GetMatchingGroup(Card.IsFaceUp,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
            if #g>0 and Duel.IsExistingMatchingCard(Card.IsExtra,tp,LOCATION_EXTRA,0,1,nil) then
                Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,1))
                local tc=g:Select(tp,1,1,nil):GetFirst()
                Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,2))
                local eq=Duel.SelectMatchingCard(tp,Card.IsExtra,tp,LOCATION_EXTRA,0,1,1,nil):GetFirst()
                Duel.Equip(tp,eq,tc)
                --Add equip limitation
                local e1=Effect.CreateEffect(c)
